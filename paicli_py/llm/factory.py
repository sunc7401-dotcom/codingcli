...
def create_client(provider: str, config: PaiCliConfig) -> LlmClient | None:
    ...
    match normalized:
        case "glm":
            from paicli_py.llm.providers.glm import GLMClient
            return GLMClient(api_key=api_key, model=model)
        case "deepseek":
            from paicli_py.llm.providers.deepseek import DeepSeekClient
            return DeepSeekClient(api_key=api_key, model=model)
        case "step":
            from paicli_py.llm.providers.step import StepClient
            return StepClient(api_key=api_key, model=model, base_url=base_url)
        case "kimi":
            from paicli_py.llm.providers.kimi import KimiClient
            return KimiClient(api_key=api_key, model=model, base_url=base_url)
        case "freellmapi":
            from paicli_py.llm.providers.free_llm_api import FreeLlmApiClient
            return FreeLlmApiClient(api_key=api_key, model=model, base_url=base_url)
        case "xfyun":
            from paicli_py.llm.providers.xfyun_maas import XfyunMaaSClient
            return XfyunMaaSClient(api_key=api_key, model=model, base_url=base_url, lora_id=lora_id)
        case "agnes":
            from paicli_py.llm.providers.agnes import AgnesClient
